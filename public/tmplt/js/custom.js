...
function SomeAddRowFunction() {
    if ($('.skill_item tr > td:contains('+$( "#select option:selected" ).text()+')').length==0) {
        $(".skill_item > tbody").append("<tr><td><div class=\"L_item\" data-id=\""+$( "#select" ).val()+"\" >"+$( "#select option:selected" ).text()+"</div></td>        <td><input type=\"button\" value=\"Delete Row\" onclick=\"SomeDeleteRowFunction(this);\"></td></tr>");
        // <td><div class=\"L_item\">"+$( "#select" ).val()+"</div></td>

        let myText= ($(".skill_item").find(".L_item").map(function() {
            return $(this).data("id");
        }).get()).join(",");
        // var els = $(".skill_item").find(".L_item").data('data-id').get();
        // var myText = els.map(function(e) { return $(e).text(); }).join(",");
